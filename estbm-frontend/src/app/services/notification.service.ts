...
export class NotificationService {
  ...
  private lastShownAt: { [key in Notification['type']]?: number } = {};
  private rateLimitMs: { [key in Notification['type']]?: number } = {
    info: 1500,
    success: 800,
    warning: 1200,
    error: 0,
  ...
  setRateLimit(type: Notification['type'], ms: number): void {
    this.rateLimitMs[type] = ms;
  }
  ...
  private show(notification: Omit<Notification, 'id'>, customId?: string): void {
    // Rate limit to avoid UI jank from excessive toasts
    const now = Date.now();
    const type = notification.type;
    const minInterval = this.rateLimitMs[type] ?? 0;
    const last = this.lastShownAt[type] ?? 0;
    if (this.quietMode && minInterval > 0 && now - last < minInterval) {
      return;
    }
    this.lastShownAt[type] = now;

    const id = customId || this.generateId();
    const newNotification: Notification = { ...notification, id };
    
    const current = this.notifications.value;
    const next = [...current, newNotification];
    // Cap total visible notifications
    while (next.length > this.maxVisible) {
      next.shift();
    }
