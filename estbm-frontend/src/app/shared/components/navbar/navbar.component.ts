...
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  private enableUiNotifications = false; // Disable noisy notifications for performance
  private scheduledScroll = false;
  ...
  ngOnInit(): void {
    ...
    // Avoid welcome toasts to keep UI snappy
  }
  ...
  onWindowScroll(): void {
    ...
    requestAnimationFrame(() => {
      const scrolled = this.lastKnownScrollY > 20;
  ...
  onKeyDown(event: KeyboardEvent): void {
    ...
    // Quick navigation shortcuts (kept but without toasts)
  ...
  private updateActiveLink(): void {
    ...
    // Intentionally silent to avoid toasts on every navigation
  }
  ...
  preloadRoute(route: string): void {
    // No-op: avoid unintended navigation that hurts UX
  }
